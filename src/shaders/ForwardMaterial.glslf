...
// #define CUBEUV_TEXEL_WIDTH 0.00260416666666673
...
float sampleVisibility( vec3 coord, float bias ) {
	...
	float depth = texture2D( depthTexture, sampleCo.xy ).r * 2.0 - 1.0;
	//depth = depth / f;
...
vec3 getIBLIrradiance( const in vec3 normal ) {
		...
		vec4 envMapColor = textureCubeUV( envMap, normal, 1.0 );
...
void main() {
	...
	vec3 v = normalize( camPos - wPosition.xyz);
	...
	if (useShadows > 0.5 && NUM_TAPS > 0)
	{
		for (int i=0; i < NUM_TAPS; i++) {
			...
			//occlusion += sampleVisibility( shadowCoord + vec3(poissonDisk[int(mod(float(i), 12.0))] * shadowRadius * (1.0 / 1024.0), 0.), 0.005);
	...
	vec3 closestPoint = L + centerToRay * clamp(sphereRadius / length(centerToRay), 0.0, 1.0);
	//L = normalize(closestPoint);
	float distLight = length(L);//length(closestPoint);
	L = normalize(L);

	gl_FragColor = vec4(BRDF(v, L, n, albedo, metallic, roughness, ao, ambientColor, ambientIntensity, occlusion, distLight, sphereRadius) , 1.0);
